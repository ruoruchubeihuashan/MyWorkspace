...
public class BisectingKMeansModel_ {
	...
	public static void main(String[] args) {
		...
		SparkConf conf = new SparkConf()
				.setMaster("local[*]")
				.setAppName("BisectingKMeansModel_")
		...
		ks.forEach(k -> {
			bkm.setK(k).setMaxIterations(numIterations);
			BisectingKMeansModel model = bkm.run(inputRDD);
			// 通过计算平方误差的集合和来评估聚类
			double WSSSE = model.computeCost(inputRDD);
			WSSSEs.put(k, WSSSE);
		});
		...
		//使用最优k值聚类		
		BisectingKMeansModel model = bkm.setK(optimize.getKey()).setMaxIterations(numIterations).run(inputRDD);
		...
		//保存最优k值的模型
		//String path = "target/models/pub/BisectingKMeans/Model/"+FORMATTER_YMDHMS.get().format(new Date());
		//model.save(jsc.sc(), path);
		//BisectingKMeansModel.load(jsc.sc(), path);
