                  ...
                  <div id="leftList" class="left_gr" >
                        ...
                        <#list basicTagList as levelOne>
	                        ...
	                        <div class="firstGr" style="transition: 0.5s;position: relative;">
	                            ...
	                            <span value='${levelOne.id}-${levelOne.level}' title='${levelOne.name}'>${levelOne.name}</span>
	                        </div>
	                        ...
	                        <div class="zi">
		                        ...
		                        <div class="secondGr">
		                        	<span class="second_img" style="background:url(${basePath}/res/imgs/left_s2.png) no-repeat 0 10px !important;" value='${levelTwo.id}-${levelTwo.level}' title=${levelTwo.name}>${levelTwo.name}</span>
		                        </div>
	                            <div class="secondzi">
	                             ...
	                             <!-- 三级标签 -->
	                              <div class="we">
	                                <div class="we_div">
	                                	<span class="we_div_span" value='${levelThree.id}-${levelThree.level}' title='${levelThree.name}' style="background:url(${basePath}/res/imgs/left_s2.png) no-repeat 0 10px !important;">${levelThree.name}</span>
	                                </div>
	                              </div>
	                              	<div class="fourth">
		                             ...
		                             <!-- 四级标签 -->
		                              <div class="four">
		                                <div class="four_div">
		                                	<span class="four_div_span" value='${levelFourth.id}-${levelFourth.level}' title='${levelFourth.name}' >${levelFourth.name}</span>
		                                </div>
		                              </div>
		                              </#list>
		                              </div>
			...
			<div style="display:block;" class="createusergroupcount_wrap">
				...
				<section class="usersureaddlabel_wrap">
					...
					<div class="usersureaddlabel_contentchoice">
						...
						<#list basictag as i> 
							<section class="sersureaddlabel_item ">
								<div class="sersureaddlabel_rec">
									...
									<p class="sersureaddlabel_rec_instruction_title" value=${i.id} title="${i.name}" style="cursor: pointer;">${i.name}</p>
								  <div class="sersureaddlabel_content  hascontent">
								  	<#list i.subTags as j> 
										<span class="sersureaddlabel_rec_instruction" value=${j.id} title="${j.name}" style="cursor: pointer;">${j.name}</span>
								    </#list>
								  </div>
									<span class="sersureaddlabel_rec_delimg"></span>
								</div>
								<select style="margin-left:5px;" class="sersureaddlabel_checkbox" onchange="formula();">
									<#if i.conditions == "and">
						...
						<div class="sersureaddlabel_oversee_watch" data-value="${mergetag.listBasicTagId}">${mergetag.condition}</div>
						<!--E 条件公式预览-->
				   </div>
					<!--E 内容-->
				</section>
					...
					<div class="usersureaddlabel_content">
						<div class="usersureaddlabel_choose">
							...
							<div class="userinputaddlabel_description">
							    <span class="userinputaddlabel_description_text">组合含义：</span><textarea id="usergroupmean" maxlength="400" class="userinputaddlabel_meaning_textarea" placeholder="最多可输入400个字符"></textarea>
							</div>
							<div class="userinputaddlabel_description">
								<span class="userinputaddlabel_description_text">组合用途：</span><textarea id="usergroupdeuse" maxlength="50" class="userinputaddlabel_description_textarea" placeholder="最多可输入50个字符"></textarea>
							</div>
							<div class="userinputaddlabel_time">
								<span class="userinputaddlabel_description_text">有效时间：</span><input type="text" id="starttime" readonly="" onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd'})" class="Wdate" style="height: 25px;border:1px solid #ccc;padding-left: 5px;width: 150px;" /><input type="text" id="endtime" readonly="" onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd'})" class="Wdate" style="height: 25px;border:1px solid #ccc;margin-left: 12px;padding-left: 5px;width: 150px;" />
							</div>
						</div>
					</div>
				</section>
