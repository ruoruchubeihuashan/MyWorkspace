...
//更新周期
  ...
  },function(xhr){layer.close(layerIndex1);layer.msg("数据服务错误！");$(".layui-layer-move").remove();})
  $("#childrenModal").modal("hide");
})
//根据父ID获取子标签
		...
		if(levelTwoName == ""){
			if(levelThreeName != ""){
				...
				if(name.test(levelOneName)  && (name.test(levelTwoName) || levelTwoName == "") && (name.test(levelThreeName) || levelThreeName == "") ){
					var labelAll = [{'name':levelOneName,'level':'1'},{'name':levelTwoName,'level':'2'},{'name':levelThreeName,'level':'3'}];
