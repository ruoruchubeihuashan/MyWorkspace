	 ...
	 /**************************点击2级出现3级标签************************************/
	  ...
	  $("#one_next").on('click',function(){
			...
			if(!flagone){
	            ...
	            if(idself != undefined && idself != null){
	        		jQuery.axsGet(true,"../merge/queryUpdateMergeTagData",{'mergeTagId':idself,'baisicLevel':1,'basicTagName':basicTagName},function(data){
	        			if(data.code == 10005){
	        				var html = '';
	        		...
	        		},function(xhr){layer.msg("数据服务错误！")})
	        	}else{
	        		jQuery.axsGet(true,"../merge/queryUpdateMergeTagData",{baisicLevel:"1","basicTagId":basicId},function(data){
		            	if(data.code == '10005' && data.data != null){
		            		var html = '';
